...
import { Webhook, Globe, Database, Twitter, Building, FileText, Clock, Zap } from 'lucide-react';
...
export default memo(({ id, data, selected }: any) => {
    const Icon = icons[data.iconType as keyof typeof icons] || Webhook;
        ...
        <div className={`relative w-48 bg-slate-900 border-2 rounded-lg shadow-2xl transition-all duration-200 ${getExecutionStyle()}`}>
            ...
            <div className="p-3">
                <div className="flex justify-between items-start mb-2">
                    <div className={`p-2 rounded-md ${data.color || 'bg-amber-400/10 text-amber-400'}`}>
                ...
                <div className="text-[10px] text-slate-500 font-mono flex justify-between items-center">
                    <span className="uppercase tracking-wider">
                        {data.triggerType === 'datasource' ? data.sourceTable : 'TRIGGER'}
                    </span>
                </div>
            </div>
