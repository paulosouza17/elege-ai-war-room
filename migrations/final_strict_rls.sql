...
-- B) The item belongs to an activation they own
...
using (
  auth.uid() = user_id 
  OR 
  activation_id in (select id from activations where user_id = auth.uid())
);
...
-- 2. Reset RLS on activations to STRICT mode
